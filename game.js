...
function randomise(){
    ...
    //Swap the numbers if number2 is greater than number1
    if(number2>number1){
        var temp = number1;
        number1 = number2;
        number2 = temp;
        // Two more ways to this:
            // 1.Destructuring method:  [number1 , number2] = [number2 , number1]
            // 2.Swapping the values:    swap(number1 , number2)
    }
    operator = Math.round(Math.random()*5); //0,1,2,3,4,5
